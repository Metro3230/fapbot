...
async def echo_message(msg: types.Message):
    ...
    add_to_log(usrid, '/root/fapbot/log.txt')
...
#    file_info = await bot.get_file(message.photo[-1].file_id)
    await message.photo[-1].download(destination_file='/root/fapbot/photo/send-' + message.photo[-1].file_unique_id + '.jpg')
    img = Image.open('/root/fapbot/photo/send-' + message.photo[-1].file_unique_id + '.jpg')
    hand = Image.open('/root/fapbot/photo/hand.png') 
    ...
    img.save('/root/fapbot/photo/send-' + message.photo[-1].file_unique_id + '_hand.jpg')       #временно сохраняем
#    await bot.send_photo(-931442766, message.photo[-1].file_id, caption=message.from_user.first_name)           #отправляем в мой другой чят
    await bot.send_photo(-931442766, message.photo[-1].file_id, caption=f'Username: {first_name}\nMsg: {caption}\nId: {usrid}')           #отправляем в мой другой чят
    await bot.send_photo(usrid, types.InputFile('/root/fapbot/photo/send-' + message.photo[-1].file_unique_id + '_hand.jpg'))   #отправляем измененное 
    add_to_log(usrid, '/root/fapbot/log.txt')
    os.remove('/root/fapbot/photo/send-' + message.photo[-1].file_unique_id + '_hand.jpg')     #удаляем
    os.remove('/root/fapbot/photo/send-' + message.photo[-1].file_unique_id + '.jpg')
